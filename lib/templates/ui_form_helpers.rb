...
module UiFormHelpers
  def ui_text_field(field_name, attributes = {}, &)
    ...
    @template.render Inputs::Field::TextComponent.new(
      field_name,
  ...
  def ui_password_field(field_name, attributes = {}, &)
    ...
    @template.render(
      Inputs::Field::TextComponent.new(field_name, form_builder: self, label:, type:, **attributes), &
    )
  end

  def ui_text_area(field_name, attributes = {}, &)
    label = options.delete(:label) { field_name.to_s.humanize }
    @template.render(Inputs::Field::TextAreaComponent.new(field_name, form_builder: self, label:, **attributes), &)
  end

  def ui_submit(value = nil, attributes = {})
    value ||= submit_default_value
    options.delete(:type)
    @template.render ButtonComponent.new(value, kind: :primary, type: :submit, **attributes)
  end
end
